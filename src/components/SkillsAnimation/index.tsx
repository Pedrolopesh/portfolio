            ...
            <FlexContainer>
                <LeftContainer>
                    {skillData.map((item: SkillType, index:number) => {
                        return(
                            <>
                                { index < 4 && <SkillBar image={item.image} percent={item.percent} skillName={item.skillName} color={item.color} /> }
                            </>
                        )
                    })}
                </LeftContainer>
                ...
                <RigthContainer>
                    {skillData.map((item: SkillType, index:number) => {
                        return(
                            <>
                                { index > 3 && <SkillBar image={item.image} percent={item.percent} skillName={item.skillName} color={item.color} /> }
                            </>
                        )
                    })}
                </RigthContainer>
